...
static void rxtx_loop_normal()
{
	...
	{
	    if (!fd_readable(the_working_paras.sockfd, 0, 10000))
			continue;
		...
		if (ret>0)
		{
			...
			if (!the_working_paras.only_recv)
			{
				...
				INC_STAT(the_stat_data.tx_bytes_total);
		...
		if (0==ret)
		{
			INC_STAT(the_stat_data.rx_times_no_data);
			continue;
		}
